 ...
 *
 * Accepts either raw VTT/text content (will parse internally) or pre-parsed
 * segments (avoids double-parsing when the caller already has segments).
 */
export function isEmptyTranscript(
  contentOrSegments: string | TranscriptSegment[],
): { empty: boolean; warning: boolean } {
  const text = typeof contentOrSegments === 'string'
    ? parseVTT(contentOrSegments)
    : contentOrSegments
        .map((s) => `${s.speaker ? s.speaker + ': ' : ''}${s.text}`)
        .join('\n');
