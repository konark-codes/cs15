...
export async function processTranscriptPayloadInternal(
  ...
  try {
    // Parse once — used for both empty check and downstream processing
    const segments = parseVTTWithSpeakers(rawContent);

    // Empty check using pre-parsed segments (avoids double-parsing)
    const { empty, warning } = isEmptyTranscript(segments);
    if (empty) {
      await ZoomMeeting.findByIdAndUpdate(meeting._id, {
        status: 'failed',
    ...
    // Derive plain text from already-parsed segments
